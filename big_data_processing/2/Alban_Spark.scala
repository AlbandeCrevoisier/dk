...
hashtags.map(w => (w, 1)).reduceByKey((a,b) => a+b).sortBy(_._2, ascending=false).take(10)

// COMMAND ----------

df.select("user").rdd.map(r => r(0).asInstanceOf[org.apache.spark.sql.Row](35)).
  map(id => (id, 1)).
  reduceByKey((a,b) => a+b).
